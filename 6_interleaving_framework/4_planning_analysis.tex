...
\begin{figure}[h]
    ...
    \caption{Example covering ball sequence for an example reference path with a distance along the path of $\ballseparation$ between each ball. Given that the path is $\deltar$-robust, each ball is a subset of $\robotCvalid$. \rev{Update figure with correct notation}}
...
\begin{figure}[h]
    ...
    \caption{Minimum domination region for a node $\pstate_i$, adapted from \citet{LiAOKP2016} Lemma 23. Sampling $\pstaterand$ in the shaded region guarantees that a node $\pstatenear \in \hyperball_{\deltar}(\pstate_k^*)$ is selected for propagation so that either $\pstatenear = \pstate_i$ or $\cost(\pstatenear) < \cost(\pstate_i)$. \rev{Update notation in this figure}}
