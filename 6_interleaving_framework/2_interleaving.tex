...
\begin{algorithmic}[1]
    ...
    \For{$n = t, \dots, t + \predictionhorizon - 1$}
        \If {Path $\neq \emptyset$}
            ...
            \State $\robotvelcmd_n \gets$ FindBestRobotMotion$(\robotconfig_n, \deformconfig_{n}, \deformvel_e, \Pinvweight_e)$
...
\begin{algorithmic}[1]
    \For {$\reducedstate = (\robotconfig, \band) \in \rrtnodeset$}
        ...
        \If {$\left\| \eeposition[0] - \eepositiongoal[0] \right\| \leq \eegoalradius$ \textbf{and} 
             $\left\| \eeposition[1] - \eepositiongoal[1] \right\| \leq \eegoalradius$ \textbf{and} 
             $\band \in \bandgoal$}
            \State \Return \texttt{true}
        \EndIf
    \EndFor
